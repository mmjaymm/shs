	...
	if (!isset($_POST["ay_start"]) == "nulls" or !isset($_POST["sem"]) == "nulls"
	 ...
	 or !isset($_POST["search"]) == "nulls" or !isset($_POST["shsid"]) == "nulls"){
      ...
      $shsid   ="";
		...
		$sql_app = mysqli_query($con,"SELECT * FROM tblstudent_advised WHERE shs_id IS NOT NULL 
		...
		$num_sql = mysqli_num_rows($sql_app);
			...
			$pdf->SetXY(5,39);
			$pdf ->SetFont('Times', 'B', 10);
			$pdf->Cell(232, 7, "LIST OF STUDENT", 1,0,'C');
			$pdf->SetTextColor(205,0,27);
			$pdf->Cell(55, 7, $num_sql."  NUMBER OF STUDENT   ", 1,0,'R');
