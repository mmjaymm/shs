<?php
	...
	//$fetch_sub_taken1 = mysqli_fetch_array($sql_sub_taken);
	//$last_advise_id = fetch_sub_taken1['advise_id'];
	...
	//==========================================================//
	// 			LAST SUBJECTS ADVISED							//
	//==========================================================//
	if($sm == "Second" && $gr == "Grade 11"){
		$sql_last_adv = mysqli_query($con,"SELECT * FROM tblstudent_advised WHERE shs_id='$shsid' AND grade='Grade 11' AND semester='First'");
		$fetch_last_adv = mysqli_fetch_array($sql_last_adv);
		$last_adv = $fetch_last_adv['advise_id'];
	}
	
	if($sm == "First" && $gr == "Grade 12"){
		$sql_last_adv = mysqli_query($con,"SELECT * FROM tblstudent_advised WHERE shs_id='$shsid' AND grade='Grade 11' AND semester='Second'");
		$fetch_last_adv = mysqli_fetch_array($sql_last_adv);
		$last_adv = $fetch_last_adv['advise_id'];
	}
	
	if($sm == "Second" && $gr == "Grade 12"){
		$sql_last_adv = mysqli_query($con,"SELECT * FROM tblstudent_advised WHERE shs_id='$shsid' AND grade='Grade 12' AND semester='First'");
		$fetch_last_adv = mysqli_fetch_array($sql_last_adv);
		$last_adv = $fetch_last_adv['advise_id'];
	}
	
	$sql_last_sub_adv = mysqli_query($con,"SELECT * FROM tblsubject_enrolled WHERE advise_id = '$last_adv'");
	$count_last_sub_adv = mysqli_num_rows($sql_last_sub_adv);
	$sql_last_sub_adv1= mysqli_query($con,"SELECT * FROM tblsubject_enrolled WHERE advise_id = '$last_adv'");
	$count_last_sub_adv1 = mysqli_num_rows($sql_last_sub_adv1);
	...
	$sql_com_grade = mysqli_query($con,"SELECT SUM(grades)
										...
										WHERE tblsubject_enrolled.advise_id = '$last_adv'
	...
	//=========================================================//
		while ($fetch_last_sub_adv = mysqli_fetch_array($sql_last_sub_adv)){
			$pdf->SetXY(10,$yx);
			$pdf->Cell(15, 5, '',1, 'B');
			$pdf->Cell(69, 5,$fetch_last_sub_adv['subdesc'],1, 'B');
			$pdf->SetFont('Arial', 'B', 8);
			$pdf->Cell(10, 5,$fetch_last_sub_adv['grades'],1, 'B','C');
			$pdf->SetFont('Arial', '', 6);
			$pdf->Ln(5);
			$a++;
			$yx += 5;
		}
		while($count_last_sub_adv<10){
			$pdf->SetXY(10,$yx);
			$pdf->Cell(15, 5,'',1, 'B');
			$pdf->Cell(69, 5,'',1, 'B');
			$pdf->Cell(10, 5,'',1, 'B','C');
	...
	$pdf->Cell(10, 6, 'Credits', 1,1,'C');
		...
		//		PARA IDISPLAY UNG NAKUHA NG SUBJECT NKARAAN SEM	   //
		//=========================================================//
		while ($fetch_last_sub_adv1 = mysqli_fetch_array($sql_last_sub_adv1)){
			$pdf->SetXY(10,$yx);
			$pdf->Cell(15, 5, '',1, 'B');
			$pdf->Cell(69, 5,$fetch_last_sub_adv1['subdesc'],1, 'B');
			$pdf->SetFont('Arial', 'B', 8);
			$pdf->Cell(10, 5,$fetch_last_sub_adv1['grades'],1, 'B','C');
			$pdf->SetFont('Arial', '', 6);
			$pdf->Ln(5);
			$a++;
			$yx += 5;
		}
		while($count_last_sub_adv1<10){
			$pdf->SetXY(10,$yx);
			$pdf->Cell(15, 5,'',1, 'B');
			$pdf->Cell(69, 5,'',1, 'B');
			$pdf->Cell(10, 5,'',1, 'B','C');
