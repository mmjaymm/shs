...
<?php
	...
	if (!isset($_POST["ay_start"]) or !isset($_POST["sem"])
	 ...
	 or !isset($_POST["search"]) or !isset($_POST["shsid"])){
      ...
      $shsid   ="";
		...
		$quer = "SELECT * FROM tblstudent_advised WHERE shs_id IS NOT NULL ".$sql_ay."".$sql_sem."".$sql_strand."".$sql_major."". $sql_grade."".$sql_section."".$sql_id; 
			
			$resultstud = mysqli_query($con,$quer);
			$trowstud   = mysqli_num_rows($resultstud);
			$rowstud    = mysqli_fetch_assoc($resultstud);    

		$query = "SELECT * FROM tblstudent_advised";
	...
	<body>
		<center>  
			<?php  echo "<h3 style='color:red;'>$trowstud Record Found</h3>"; ?>
    ...
    ?>
					<tr>
						<td style = "text-align:center;">CCT-SHS-<?php printf ("%s",strtoupper ($rowstud["shs_id"]));?></td>
